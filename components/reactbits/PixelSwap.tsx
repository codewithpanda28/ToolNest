...
type PixelPattern = (x: number, y: number) => number | null;
...
const PATTERNS: Record<PatternName, PixelPattern> = {
  random: () => null,
...
const makeEasing = (value: string): EasingFn => {
...
const buildKeyframes = ({
  ...
  spin: number;
  fade: boolean;
}): { window: KeyframeStep[]; content: KeyframeStep[] } => {
  const window: KeyframeStep[] = [];
  const content: KeyframeStep[] = [];
